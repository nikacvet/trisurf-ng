...
ts_vertex._fields_=[
		...
		('direct_interaction_force',c_double)
		]
...
class ts_confinement_plane(Structure):
	_fields = [
		('z_max',c_float),
		('z_min',c_float),
		('force_switch',c_int)
...
class ts_tape(Structure):
	_fields_=[
		...
		('w', c_double),
		('F', c_double),
		('plane_confinement_switch',c_long),
		('plane_d',c_double),
		('plane_F',c_double)
	]
...
class ts_vesicle(Structure):
	_fields_=[
		...
		('area', c_double),
		('confinement_plane',POINTER(ts_confinement_plane))
	]
...
class ts_args(Structure):
	_fields = [
		...
		('tape_opts',c_char*1000),
		('dump_from_vtk',c_char*1024)
	]

class ts_string(Structure):
	_fields = [('string',c_char_p), ('beg',c_uint)]
